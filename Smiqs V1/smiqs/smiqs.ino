...
void loop() {
  ...
  if (mqtt_client->connected() && millis() - lastTelemetryMillis > telemetrySendInterval) {
    ...
    String payload = F("{\"SSA\": {SSA},\"SSB\": {SSB},\"TVWCMax\": {VWCMax},\"TVWCMin\": {VWCMin},\"VA\": {VA}}");
  ...
  if (mqtt_client->connected() && millis() - lastPropertyMillis > PROPERTY_SEND_INTERVAL) {
    ...
    String payload = F("{\"VWCMin\": {VWCMin}, \"VWCMax\": {VWCMax}}");
    payload.replace(F("{VWCMin}"), String(SolenoidValve_valSensorMin));
    payload.replace(F("{VWCMax}"), String(SolenoidValve_valSensorMax));
  ...
  if ( VarVWCA > 5.0 &&  VarVWCA >= SolenoidValve_valSensorMax && !ManualOverride)
  {
    ...
    if (sent == false) {
      ...
      totalMilliLitres = (pulseCount * 2.25) / 1000;
      sms.beginSMS(MobileNumber);
      sms.print("Status is Good, Happy Garden :) Duration of watering is: " + String(elapsed / 1000) + " s. Garden Health is:" + String(VarVWCA) + ". Garden Temperature is: " + String(readTH200()) + " C. Output water is :" + String(totalMilliLitres) + " L");
      sms.endSMS();
      sent = true;
      ...
      if (mqtt_client->connected()) {
        ...
        String topic = (String)IOT_EVENT_TOPIC;
        topic.replace(F("{device_id}"), deviceId);
        String payload = F("{\"SSA\": {SSA},\"SSB\": {SSB},\"ELT\": {ELT},\"TVWCMax\": {VWCMax},\"FML\": {FML},\"TVWCMin\": {VWCMin},\"VA\": {VA}}");
        payload.replace(F("{SSA}"), String(VarVWCA));
        payload.replace(F("{SSB}"), String(VarSoilTemp));
        payload.replace(F("{VWCMax}"), String(SolenoidValve_valSensorMax));
        payload.replace(F("{VWCMin}"), String(SolenoidValve_valSensorMin));
        payload.replace(F("{ELT}"), String(elapsed / 1000));
        payload.replace(F("{FML}"), String(totalMilliLitres));
        payload.replace(F("{VA}"), String(1));
        Serial_printf("\t%s\n", payload.c_str());
        mqtt_client->publish(topic.c_str(), payload.c_str());
      }
      pulseCount = 0;
  ...
  if (VarVWCA > 5.0 && VarVWCA < SolenoidValve_valSensorMin  && !ManualOverride)
  {
    ...
    //  elapsed = timenow - start;  // elapsed: duration
    //Open Valve
    ...
    if (sent == true) {
      ...
      start = millis();
      sms.beginSMS(MobileNumber);
      sms.print("Plant needs water. Current health is : " + String(VarVWCA) + " .Irrigation Started.");
      sms.endSMS();
      sent = false;
      if (mqtt_client->connected()) {
